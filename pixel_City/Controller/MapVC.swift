...
class MapVC: UIViewController, UIGestureRecognizerDelegate {
  ...
  @IBAction func pixelMapBtnWasPressed(_ sender: Any) {
...
extension MapVC: MKMapViewDelegate{
  ...
  @objc func dropPin(sender: UITapGestureRecognizer){
    ...
    let annotatiation = DroppablePin(coordinate: touchCoordinate, identifier: "droppablePin")
    mapView.addAnnotation(annotatiation)
    print(flickerUrl(forApiKey: apiKey, withAnnotation: annotatiation, andNumberOfFhotos: 40))
    
    let cordinateRegion = MKCoordinateRegion(center: touchCoordinate, latitudinalMeters: regionRadius * 2.0, longitudinalMeters: regionRadius * 2.0)
    mapView.setRegion(cordinateRegion, animated: true)
    retriveUrls(forAnnotation: annotatiation) { (finished) in
      if finished{
  ...
  func retriveUrls(forAnnotation annotation: DroppablePin, handler: @escaping(_ status: Bool) ->()){
    Alamofire.request(flickerUrl(forApiKey: apiKey, withAnnotation: annotation, andNumberOfFhotos: 40)).responseJSON { (response) in
